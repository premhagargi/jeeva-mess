    ...
    <div className="space-y-4 sm:space-y-6">
      <header className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 bg-secondary px-2.5 sm:px-3 py-1.5 border border-border shrink-0">
          ...
          <span className="text-xs sm:text-xs font-bold">{orders.length} Total</span>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {orders.length === 0 ? (
          <div className="col-span-full py-16 sm:py-20 text-center border border-dashed border-border bg-secondary/20">
            <p className="concierge-text text-muted-foreground text-xs">Queue is empty</p>
          </div>
        ) : (
          orders.map((order) => (
            <div key={order.id} className="bg-white border border-border p-3 sm:p-4 flex flex-col justify-between hover:border-accent transition-all active:border-accent">
              <div className="flex justify-between items-start mb-2 gap-2">
                <div className="min-w-0">
                  <h3 className="font-bold text-sm text-accent truncate">{order.id}</h3>
                  <p className="text-[13px] font-bold truncate">{order.studentName}</p>
                  <p className="text-xs font-bold text-muted-foreground">ID: {order.studentId}</p>
                </div>
